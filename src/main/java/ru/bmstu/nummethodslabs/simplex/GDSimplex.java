...
public class GDSimplex {
    ...
    public GDSimplex(double[][] A, double[] b, double[] c) {
        M = b.length;
        N = c.length;
    ...
    public static void test(double[][] A, double[] b, double[] c) {
        GDSimplex lp = new GDSimplex(A, b, c);
        System.out.println("value = " + lp.value());
        double[] x = lp.primal();
        double[] y = lp.dual();


        /*!!!*/for (int j = 0; j < y.length; j++) x[x.length - j - 1] = y[j];
